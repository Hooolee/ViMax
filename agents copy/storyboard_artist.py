...
class StoryboardArtist:
    ...
    async def design_storyboard(
        ...
        # 记录提示词到日志文件
        log_agent_prompt(
            agent_name="StoryboardArtist",
            prompt_type="system",
            prompt_content=messages[0][1],
            metadata={"method": "design_storyboard", "model": str(self.chat_model)}
        )
        log_agent_prompt(
            agent_name="StoryboardArtist",
            prompt_type="human",
            prompt_content=messages[1][1],
            metadata={"method": "design_storyboard"}
        )
    ...
    async def decompose_visual_description(
        ...
        log_agent_prompt(
            ...
            metadata={"method": "decompose_visual_description", "model": str(self.chat_model)}
        )
        log_agent_prompt(
            agent_name="StoryboardArtist",
            prompt_type="human",
            prompt_content=formatted_messages[1].content,
            metadata={"method": "decompose_visual_description"}
        )
